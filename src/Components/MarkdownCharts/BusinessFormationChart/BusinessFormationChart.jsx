...
const BusinessFormation = ({ dataPath, config }) => {
  ...
  const [selectedLocation, setSelectedLocation] = React.useState("MPO");
  ...
  React.useEffect(() => {
    csv(dataPath).then((csvData) => {
      const processedData = csvData.map((row) => ({
        ...
        fiveyears: parseFloat(row[`${selectedLocation}_5yr`]) || 0,
      }));
    ...
    <div>
      ...
      <ResponsiveContainer width="90%" height={500}>
        ...
        >
          ...
          <Line
            type="monotone"
            dataKey={viewType === "fiveyears" ? "fiveyears_avg" : "total_avg"}
